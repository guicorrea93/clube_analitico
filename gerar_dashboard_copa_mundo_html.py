...
.subnav{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 14px}.subnav button{color:var(--text);background:#10251c;border:1px solid var(--line);border-radius:8px;padding:8px 12px;cursor:pointer;font-size:13px}.subnav button.active{background:var(--gold);border-color:var(--gold);color:#11170c;font-weight:800}.goal-pane.hidden,.game-pane.hidden{display:none}
...
.goals-toolbar{grid-template-columns:repeat(2,minmax(180px,260px))}.jogos-toolbar{grid-template-columns:repeat(3,minmax(180px,260px))}.goals-table,.games-table{max-height:520px}.quality-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.quality-item{border:1px solid var(--line);border-radius:8px;background:rgba(7,19,15,.42);padding:12px}.quality-item b{display:block;font-size:25px;line-height:1.05}.quality-item span{display:block;margin-top:5px;color:var(--muted);font-size:12px;line-height:1.35}
...
<nav class="nav" id="nav"><button data-view="geral" class="active">Geral das Copas</button><button data-view="gols">Gols</button><button data-view="jogos">Jogos</button><button data-view="copa">Por Copa</button><button data-view="selecao">Por Seleção</button></nav>
<main class="shell">
  ...
  <section id="view-gols" class="hidden">
    ...
    <div class="subnav" id="goals-subnav">
      <button data-goal-sub="panorama" class="active">Panorama</button>
      <button data-goal-sub="selecoes">Seleções</button>
      <button data-goal-sub="jogadores">Jogadores</button>
      <button data-goal-sub="contexto">Contexto</button>
      <button data-goal-sub="curiosidades">Curiosidades</button>
      <button data-goal-sub="qualidade">Qualidade</button>
      <button data-goal-sub="base">Base</button>
    </div>
    <div class="grid">
      <article class="card span-6 goal-pane goal-pane-panorama"><h2>Gols por edição</h2><div id="ch-goals-edicao" class="chart small"></div></article>
      <article class="card span-6 goal-pane goal-pane-panorama"><h2>Gols por seleção</h2><div class="chart-scroll chart-scroll-x"><div id="ch-goals-teams" class="chart"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-selecoes hidden"><h2>Eficiência ofensiva</h2><div class="chart-scroll"><div id="ch-goals-off-eff" class="chart"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-selecoes hidden"><h2>Frequência ofensiva</h2><div class="chart-scroll"><div id="ch-goals-scoring-frequency" class="chart"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-selecoes hidden"><h2>Eficiência defensiva</h2><div class="chart-scroll"><div id="ch-goals-def-eff" class="chart"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-selecoes hidden"><h2>Sequências por seleção</h2><div class="table-wrap goals-table"><table id="tbl-goals-streaks"></table></div></article>
      <article class="card span-6 goal-pane goal-pane-selecoes hidden"><h2>Gols marcados, sofridos e saldo</h2><div class="chart-scroll"><div id="ch-goals-team-balance" class="chart"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-selecoes hidden"><h2>Médias por seleção</h2><div class="chart-scroll"><div id="ch-goals-team-averages" class="chart"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-jogadores hidden"><h2>Maiores artilheiros</h2><div class="chart-scroll"><div id="ch-goals-players" class="goal-rank"></div></div><span class="note">Lista todos os jogadores com pelo menos um gol na base filtrada.</span></article>
      <article class="card span-6 goal-pane goal-pane-jogadores hidden"><h2>Artilheiro por Copa</h2><div class="chart-scroll"><div id="ch-goals-topscorers-edicao" class="goal-rank"></div></div><span class="note">Quando houve empate na artilharia, a edição aparece com todos os nomes consolidados na mesma barra.</span></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Gols por fase</h2><div id="ch-goals-phase" class="chart small"></div></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Gols/jogo por fase</h2><div id="ch-goals-phase-rate" class="chart small"></div></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Gols em mata-mata por seleção</h2><div class="chart-scroll"><div id="ch-goals-ko-teams" class="goal-rank"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Gols em mata-mata por jogador</h2><div class="chart-scroll"><div id="ch-goals-ko-players" class="goal-rank"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Seleções que mais abriram o placar</h2><div class="chart-scroll"><div id="ch-goals-first-teams" class="goal-rank"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Gols finais e acréscimos</h2><div class="chart-scroll"><div id="ch-goals-late-teams" class="chart"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Peso do gol por seleção</h2><div class="chart-scroll"><div id="ch-goals-impact-teams" class="chart"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Jogadores com gols decisivos</h2><div class="chart-scroll"><div id="ch-goals-decisive-players" class="goal-rank"></div></div></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Tipos de impacto</h2><div id="ch-goals-impact-types" class="chart small"></div></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Gols decisivos por fase</h2><div id="ch-goals-decisive-phase" class="chart small"></div></article>
      <article class="card span-6 goal-pane goal-pane-contexto hidden"><h2>Gols por faixa de minuto</h2><div id="ch-goals-minute" class="chart small"></div></article>
      <article class="card span-6 goal-pane goal-pane-curiosidades hidden"><h2>Mata-mata mais goleador</h2><div id="ch-goals-ko-editions" class="chart small"></div></article>
      <article class="card span-6 goal-pane goal-pane-curiosidades hidden"><h2>Gols tardios por edição</h2><div id="ch-goals-late-editions" class="chart small"></div></article>
      <article class="card span-6 goal-pane goal-pane-curiosidades hidden"><h2>Jogo com mais gols por edição</h2><div class="table-wrap goals-table"><table id="tbl-goals-record-matches"></table></div></article>
      <article class="card span-6 goal-pane goal-pane-curiosidades hidden"><h2>Finais mais goleadoras</h2><div class="table-wrap goals-table"><table id="tbl-goals-finals"></table></div></article>
      <article class="card span-6 goal-pane goal-pane-curiosidades hidden"><h2>Campeões mais econômicos</h2><div class="table-wrap goals-table"><table id="tbl-goals-champions"></table></div></article>
      <article class="card span-6 goal-pane goal-pane-curiosidades hidden"><h2>Artilheiros que caíram mais cedo</h2><div class="table-wrap goals-table"><table id="tbl-goals-topscorer-exits"></table></div></article>
      <article class="card span-6 goal-pane goal-pane-qualidade hidden"><h2>Cobertura dos gols por edição</h2><div id="ch-goals-coverage-editions" class="chart small"></div></article>
      <article class="card span-6 goal-pane goal-pane-qualidade hidden"><h2>Lacunas por edição</h2><div id="ch-goals-missing-editions" class="chart small"></div></article>
      <article class="card span-6 goal-pane goal-pane-qualidade hidden"><h2>Qualidade por edição</h2><div class="table-wrap goals-table"><table id="tbl-goals-quality-editions"></table></div></article>
      <article class="card span-6 goal-pane goal-pane-qualidade hidden"><h2>Divergências placar x marcadores</h2><div class="table-wrap goals-table"><table id="tbl-goals-score-divergences"></table></div></article>
      <article class="card span-6 goal-pane goal-pane-qualidade hidden"><h2>Jogadores não identificados</h2><div class="table-wrap goals-table"><table id="tbl-goals-unknown-players"></table></div></article>
      <article class="card span-6 goal-pane goal-pane-qualidade hidden"><h2>Resumo de lacunas</h2><div id="goals-quality-summary" class="quality-summary"></div></article>
      <article class="card span-12 goal-pane goal-pane-base hidden"><h2>Todos os gols</h2><div class="table-wrap goals-table"><table id="tbl-goals"></table></div></article>
    </div>
  </section>

  <section id="view-jogos" class="hidden">
    <div class="toolbar jogos-toolbar">
      <label>Ano<select id="game-year"></select></label>
      <label>Seleção<select id="game-team"></select></label>
      <label>Fase<select id="game-phase"></select></label>
    </div>
    <div class="kpis" id="kpis-jogos"></div>
    <div class="subnav" id="games-subnav">
      <button data-game-sub="panorama" class="active">Panorama</button>
      <button data-game-sub="selecoes">Seleções</button>
      <button data-game-sub="contexto">Contexto</button>
      <button data-game-sub="confrontos">Confrontos</button>
      <button data-game-sub="sequencias">Sequências</button>
      <button data-game-sub="base">Base</button>
    </div>
    <div class="grid">
      <article class="card span-6 game-pane game-pane-panorama"><h2>Jogos por edição</h2><div id="ch-games-editions" class="chart small"></div></article>
      <article class="card span-6 game-pane game-pane-panorama"><h2>Resultados por edição</h2><div id="ch-games-results-editions" class="chart small"></div></article>
      <article class="card span-6 game-pane game-pane-panorama"><h2>Distribuição de placares</h2><div id="ch-games-score-profile" class="chart small"></div></article>
      <article class="card span-6 game-pane game-pane-panorama"><h2>Margem de vitória</h2><div id="ch-games-margins" class="chart small"></div></article>
      <article class="card span-6 game-pane game-pane-selecoes hidden"><h2>Vitórias, empates e derrotas</h2><div class="chart-scroll"><div id="ch-games-team-results" class="chart"></div></div></article>
      <article class="card span-6 game-pane game-pane-selecoes hidden"><h2>Aproveitamento por seleção</h2><div class="chart-scroll"><div id="ch-games-team-points" class="chart"></div></div></article>
      <article class="card span-6 game-pane game-pane-selecoes hidden"><h2>Invencibilidade e vitórias</h2><div class="chart-scroll"><div id="ch-games-team-streaks" class="chart"></div></div></article>
      <article class="card span-6 game-pane game-pane-selecoes hidden"><h2>Eficiência por seleção</h2><div class="table-wrap games-table"><table id="tbl-games-team-efficiency"></table></div></article>
      <article class="card span-6 game-pane game-pane-contexto hidden"><h2>Jogos por fase</h2><div id="ch-games-phase" class="chart small"></div></article>
      <article class="card span-6 game-pane game-pane-contexto hidden"><h2>Resultados por fase</h2><div id="ch-games-phase-results" class="chart small"></div></article>
      <article class="card span-6 game-pane game-pane-contexto hidden"><h2>Prorrogação e pênaltis</h2><div id="ch-games-extra" class="chart small"></div></article>
      <article class="card span-6 game-pane game-pane-contexto hidden"><h2>Empates por edição</h2><div id="ch-games-draws-editions" class="chart small"></div></article>
      <article class="card span-6 game-pane game-pane-confrontos hidden"><h2>Confrontos mais repetidos</h2><div class="table-wrap games-table"><table id="tbl-games-head-to-head"></table></div></article>
      <article class="card span-6 game-pane game-pane-confrontos hidden"><h2>Maiores vitórias</h2><div class="table-wrap games-table"><table id="tbl-games-biggest-wins"></table></div></article>
      <article class="card span-6 game-pane game-pane-confrontos hidden"><h2>Adversários com mais vitórias</h2><div class="table-wrap games-table"><table id="tbl-games-opponent-wins"></table></div></article>
      <article class="card span-6 game-pane game-pane-confrontos hidden"><h2>Jogos mais frequentes por seleção</h2><div class="table-wrap games-table"><table id="tbl-games-team-opponents"></table></div></article>
      <article class="card span-6 game-pane game-pane-sequencias hidden"><h2>Maiores sequências</h2><div class="table-wrap games-table"><table id="tbl-games-streaks"></table></div></article>
      <article class="card span-6 game-pane game-pane-sequencias hidden"><h2>Sequências sem vencer e sem marcar</h2><div class="table-wrap games-table"><table id="tbl-games-negative-streaks"></table></div></article>
      <article class="card span-12 game-pane game-pane-base hidden"><h2>Todos os jogos</h2><div class="table-wrap games-table"><table id="tbl-games"></table></div></article>
    </div>
  </section>
...
let state = {view:"geral", cup:2022, q:"", phase:"", cupTeam:"", selectedMatch:"", team:"Brasil", teamQ:"", teamSort:"titulos", teamLimit:25, rankMode:"total", goalYear:"", goalTeam:"", goalSub:"panorama", goalType:"", goalBody:"", goalZone:"", gameYear:"", gameTeam:"", gamePhase:"", gameSub:"panorama"};
...
function init(){
  ...
  initRankTips();
  renderTrofeus(); renderGeral(); renderGols(); renderJogos(); renderCopa(); renderSelecao();
}
function switchView(){document.querySelectorAll(".nav button").forEach(b=>b.classList.toggle("active",b.dataset.view===state.view));["geral","gols","jogos","copa","selecao"].forEach(v=>document.getElementById("view-"+v).classList.toggle("hidden",v!==state.view)); document.getElementById("hero-trofeus").classList.toggle("hidden",state.view!=="geral"); document.getElementById("hero-cartaz").classList.toggle("hidden",state.view!=="copa"); document.getElementById("hero-bandeira").classList.toggle("hidden",state.view!=="selecao"); setTimeout(()=>{ if(state.view==="geral") renderGeral(); if(state.view==="gols") renderGols(); if(state.view==="jogos") renderJogos(); if(state.view==="copa") renderCopa(); if(state.view==="selecao") renderSelecao(); window.dispatchEvent(new Event("resize")); },20);}
...
function initGoalTabs(){
  const nav=document.getElementById("goals-subnav");
  nav.onclick=e=>{
    const btn=e.target.closest("[data-goal-sub]");
    if(!btn) return;
    state.goalSub=btn.dataset.goalSub;
    renderGoalTabs();
    renderGols();
...
function renderGoalTabs(){
  document.querySelectorAll("[data-goal-sub]").forEach(b=>b.classList.toggle("active",b.dataset.goalSub===state.goalSub));
  document.querySelectorAll(".goal-pane").forEach(el=>el.classList.toggle("hidden",!el.classList.contains("goal-pane-"+state.goalSub)));
}
...
function renderGols(){
  ...
  renderGoalTeamEfficiencyCharts();
  renderGoalTeamBalanceCharts();
  renderGoalPlayersAll(rows);
  renderGoalsTopScorers();
  renderGoalKnockoutRankings(rows);
  renderGoalContextCharts(rows);
  renderGoalCuriosities(rows);
  renderGoalQuality(rows);
...
function initGameFilters(){
  fillGameSelect("game-year", [["","Todos os anos"], ...DATA.copas.map(c=>c.ano).sort((a,b)=>b-a).map(y=>[y,y])], "gameYear");
  const teams=uniq(DATA.copas.flatMap(c=>c.partidas.flatMap(p=>[p.time1,p.time2]))).sort((a,b)=>a.localeCompare(b,"pt-BR"));
  fillGameSelect("game-team", [["","Todas as seleções"], ...teams.map(t=>[t,t])], "gameTeam");
  const phases=uniq(DATA.copas.flatMap(c=>c.partidas.map(p=>phaseLabel(p.fase)))).sort(phaseSort);
  fillGameSelect("game-phase", [["","Todas as fases"], ...phases.map(p=>[p,p])], "gamePhase");
}
function fillGameSelect(id, rows, key){
  fillSelect(id, rows);
  document.getElementById(id).value = state[key] || "";
  document.getElementById(id).onchange = e => { state[key]=e.target.value; renderJogos(); };
}
function initGameTabs(){
  const nav=document.getElementById("games-subnav");
  nav.onclick=e=>{
    const btn=e.target.closest("[data-game-sub]");
    if(!btn) return;
    state.gameSub=btn.dataset.gameSub;
    renderGameTabs();
    renderJogos();
    setTimeout(()=>window.dispatchEvent(new Event("resize")),20);
  };
...
function renderGameTabs(){
  document.querySelectorAll("[data-game-sub]").forEach(b=>b.classList.toggle("active",b.datasetGameSub===state.gameSub || b.dataset.gameSub===state.gameSub));
  document.querySelectorAll(".game-pane").forEach(el=>el.classList.toggle("hidden",!el.classList.contains("game-pane-"+state.gameSub)));
}
function allGames(){
  const out=[];
  DATA.copas.forEach(c=>c.partidas.forEach((p,i)=>{
    const s=scoreParts(p.placar);
    if(s[0]==="" || s[1]==="") return;
    out.push({...p,ano:c.ano,idx:i,g1:num(s[0]),g2:num(s[1]),fase_normalizada:phaseLabel(p.fase)});
  }));
  return out;
}
function filteredGames(){
  return allGames().filter(p=>
    (!state.gameYear || String(p.ano)===String(state.gameYear)) &&
    (!state.gameTeam || p.time1===state.gameTeam || p.time2===state.gameTeam) &&
    (!state.gamePhase || p.fase_normalizada===state.gamePhase)
  );
}
function gameResult(p){
  if(p.g1>p.g2) return "Vitória";
  if(p.g2>p.g1) return "Vitória";
  return "Empate";
}
function gameResultForTeam(p,t){
  const gf=p.time1===t?p.g1:p.g2, ga=p.time1===t?p.g2:p.g1;
  if(gf>ga) return "Vitória";
  if(gf<ga) return "Derrota";
  return "Empate";
}
function gameStatsByTeam(rows=filteredGames()){
  const stats={};
  rows.forEach(p=>{
    [[p.time1,p.g1,p.g2],[p.time2,p.g2,p.g1]].forEach(([team,gp,gc])=>{
      if(!team || (state.gameTeam && team!==state.gameTeam)) return;
      const r=stats[team] ||= {team,j:0,v:0,e:0,d:0,gp:0,gc:0,sg:0,pts:0,clean:0,marcou:0,extra:0,pen:0};
      r.j+=1; r.gp+=gp; r.gc+=gc; r.sg+=gp-gc;
      if(gp>gc){r.v+=1; r.pts+=3;} else if(gp<gc){r.d+=1;} else {r.e+=1; r.pts+=1;}
      if(gc===0) r.clean+=1;
      if(gp>0) r.marcou+=1;
      if(isExtraTime(p)) r.extra+=1;
      if(isPenaltyMatch(p)) r.pen+=1;
    });
  });
  return Object.values(stats).map(r=>({
    ...r,
    aproveitamento:r.j?+(r.pts/(r.j*3)*100).toFixed(1):0,
    pctV:r.j?+(r.v/r.j*100).toFixed(1):0,
    pctE:r.j?+(r.e/r.j*100).toFixed(1):0,
    pctD:r.j?+(r.d/r.j*100).toFixed(1):0,
    gpj:r.j?+(r.gp/r.j).toFixed(2):0,
    gcj:r.j?+(r.gc/r.j).toFixed(2):0,
    cleanPct:r.j?+(r.clean/r.j*100).toFixed(1):0,
  }));
}
function isDrawMatch(p){return p.g1===p.g2;}
function isExtraTime(p){return /\bpro\b|prorroga/i.test(String(p.placar||""));}
function isPenaltyMatch(p){return /pen/i.test(String(p.placar||""));}
function gameMarginBucket(p){
  const m=Math.abs(p.g1-p.g2);
  if(m===0) return "Empate";
  if(m>=5) return "5+ gols";
  return `${m} gol${m>1?"s":""}`;
}
function scoreProfile(p){
  const a=Math.min(p.g1,p.g2), b=Math.max(p.g1,p.g2);
  return `${b}-${a}`;
}
function renderJogos(){
  const rows=filteredGames();
  const teamRows=gameStatsByTeam(rows);
  const goals=rows.reduce((s,p)=>s+p.g1+p.g2,0);
  const draws=rows.filter(isDrawMatch).length;
  const winners=rows.length-draws;
  const extras=rows.filter(isExtraTime).length;
  const pens=rows.filter(isPenaltyMatch).length;
  if(state.gameTeam){
    const r=teamRows[0] || {j:0,v:0,e:0,d:0,aproveitamento:0,gp:0,gc:0};
    document.getElementById("kpis-jogos").innerHTML=kpis([["Jogos",r.j],["Vitórias",r.v],["Empates",r.e],["Derrotas",r.d],["Aproveitamento",`${r.aproveitamento.toFixed(1).replace(".",",")}%`],["Saldo",r.sg]]);
  }else{
    document.getElementById("kpis-jogos").innerHTML=kpis([["Jogos na visão",rows.length],["Com vencedor",winners],["Empates",draws],["Gols",goals],["Gols/jogo",rows.length?(goals/rows.length).toFixed(2).replace(".",","):"0,00"],["Pênaltis",pens]]);
  }
  renderGameEditionCharts(rows);
  renderGameScoreCharts(rows);
  renderGameTeamCharts(teamRows);
  renderGameContextCharts(rows);
  renderGameTables(rows, teamRows);
  renderGameTabs();
}
function renderGameEditionCharts(rows){
  const byEdition=Object.entries(groupBy(rows,p=>p.ano)).map(([ano,list])=>({
    ano:+ano,
    jogos:list.length,
    comVencedor:list.filter(p=>!isDrawMatch(p)).length,
    empates:list.filter(isDrawMatch).length,
    gols:list.reduce((s,p)=>s+p.g1+p.g2,0),
  })).sort((a,b)=>a.ano-b.ano);
  const x=byEdition.map(r=>String(r.ano));
  Plotly.react("ch-games-editions",[
    {type:"bar",x,y:byEdition.map(r=>r.jogos),name:"Jogos",marker:{color:COLORS[2],cornerradius:4},text:byEdition.map(r=>r.jogos),textposition:"outside",cliponaxis:false,hovertemplate:"<b>%{x}</b><br>Jogos: %{y}<extra></extra>"},
    {type:"scatter",mode:"lines+markers",x,y:byEdition.map(r=>r.jogos?+(r.gols/r.jogos).toFixed(2):0),yaxis:"y2",name:"Gols/jogo",line:{color:COLORS[1],width:3},marker:{size:6},hovertemplate:"<b>%{x}</b><br>Gols/jogo: %{y:.2f}<extra></extra>"}
  ],layout({hovermode:"closest",margin:{l:44,r:52,t:10,b:70},xaxis:{type:"category",tickangle:-25,automargin:true},yaxis:{title:"Jogos"},yaxis2:{title:"Gols/jogo",overlaying:"y",side:"right",gridcolor:"rgba(0,0,0,0)",tickformat:".2f"},legend:{orientation:"h",y:-.2}}),PCFG);
  Plotly.react("ch-games-results-editions",[
    {type:"bar",x,y:byEdition.map(r=>r.comVencedor),name:"Com vencedor",marker:{color:COLORS[0],cornerradius:4},hovertemplate:"<b>%{x}</b><br>Com vencedor: %{y}<extra></extra>"},
    {type:"bar",x,y:byEdition.map(r=>r.empates),name:"Empates",marker:{color:COLORS[2],cornerradius:4},hovertemplate:"<b>%{x}</b><br>Empates: %{y}<extra></extra>"},
  ],layout({barmode:"stack",hovermode:"closest",margin:{l:44,r:18,t:10,b:70},xaxis:{type:"category",tickangle:-25,automargin:true},yaxis:{title:"Jogos"},legend:{orientation:"h",y:-.2}}),PCFG);
}
function renderGameScoreCharts(rows){
  const scoreRows=countRows(rows,scoreProfile).slice(0,18);
  renderGoalBar("ch-games-score-profile", scoreRows, "Jogos", COLORS[1]);
  const marginOrder=["Empate","1 gol","2 gols","3 gols","4 gols","5+ gols"];
  const margins=countBy(rows,gameMarginBucket);
  renderGoalBar("ch-games-margins", marginOrder.map(k=>[k,margins[k]||0]), "Jogos", COLORS[2]);
}
function renderGameTeamCharts(teamRows){
  const rows=teamRows.slice().sort((a,b)=>b.j-a.j || b.v-a.v || a.team.localeCompare(b.team,"pt-BR"));
  renderTeamMetricGrouped("ch-games-team-results", rows, [
    {key:"v",name:"Vitórias",color:COLORS[0]},
    {key:"e",name:"Empates",color:COLORS[2]},
    {key:"d",name:"Derrotas",color:COLORS[3]},
  ], "Jogos");
  renderMetricRankBars("ch-games-team-points", rows, "aproveitamento", "Aproveitamento", COLORS[1], {decimals:1,suffix:"%"});
  renderTeamMetricGrouped("ch-games-team-streaks", gameStreakRows().sort((a,b)=>b.invicto-a.invicto || b.vitorias-a.vitorias || a.team.localeCompare(b.team,"pt-BR")), [
    {key:"invicto",name:"Maior seq. invicta",color:COLORS[0]},
    {key:"vitorias",name:"Maior seq. vitórias",color:COLORS[1]},
    {key:"semSofrer",name:"Maior seq. sem sofrer",color:COLORS[2]},
  ], "Jogos");
}
function renderGameContextCharts(rows){
  const phases=Object.entries(groupBy(rows,p=>p.fase_normalizada)).map(([fase,list])=>({fase,jogos:list.length,comVencedor:list.filter(p=>!isDrawMatch(p)).length,empates:list.filter(isDrawMatch).length})).sort((a,b)=>phaseSort(a.fase,b.fase));
  Plotly.react("ch-games-phase",[{type:"bar",x:phases.map(r=>r.fase),y:phases.map(r=>r.jogos),marker:{color:COLORS[0],cornerradius:4},text:phases.map(r=>r.jogos),textposition:"outside",cliponaxis:false,hovertemplate:"<b>%{x}</b><br>Jogos: %{y}<extra></extra>"}],layout({hovermode:"closest",margin:{l:44,r:18,t:10,b:70},xaxis:{type:"category",tickangle:-20,automargin:true},yaxis:{title:"Jogos"}}),PCFG);
  Plotly.react("ch-games-phase-results",[
    {type:"bar",x:phases.map(r=>r.fase),y:phases.map(r=>r.comVencedor),name:"Com vencedor",marker:{color:COLORS[0],cornerradius:4},hovertemplate:"<b>%{x}</b><br>Com vencedor: %{y}<extra></extra>"},
    {type:"bar",x:phases.map(r=>r.fase),y:phases.map(r=>r.empates),name:"Empates",marker:{color:COLORS[2],cornerradius:4},hovertemplate:"<b>%{x}</b><br>Empates: %{y}<extra></extra>"},
  ],layout({barmode:"stack",hovermode:"closest",margin:{l:44,r:18,t:10,b:70},xaxis:{type:"category",tickangle:-20,automargin:true},yaxis:{title:"Jogos"},legend:{orientation:"h",y:-.2}}),PCFG);
  renderGoalBar("ch-games-extra", [["Tempo normal",rows.filter(p=>!isExtraTime(p)&&!isPenaltyMatch(p)).length],["Prorrogação",rows.filter(isExtraTime).length],["Pênaltis",rows.filter(isPenaltyMatch).length]], "Jogos", COLORS[1]);
  const drawsByEdition=Object.entries(groupBy(rows,p=>p.ano)).map(([ano,list])=>[ano,list.filter(isDrawMatch).length]).sort((a,b)=>+a[0]-+b[0]);
  renderGoalBar("ch-games-draws-editions", drawsByEdition, "Empates", COLORS[2]);
}
function renderGameTables(rows, teamRows){
  table("tbl-games-team-efficiency",["Seleção","J","V","E","D","Pts","Aprov.","GP","GC","SG","Clean sheets"],teamRows.sort((a,b)=>b.aproveitamento-a.aproveitamento || b.j-a.j || a.team.localeCompare(b.team,"pt-BR")).map(r=>[r.team,r.j,r.v,r.e,r.d,r.pts,`${r.aproveitamento.toFixed(1).replace(".",",")}%`,r.gp,r.gc,r.sg,r.clean]),[1,2,3,4,5,7,8,9,10]);
  const h2h=Object.entries(groupBy(rows,p=>[p.time1,p.time2].sort((a,b)=>a.localeCompare(b,"pt-BR")).join(" x "))).map(([jogo,list])=>({jogo,j:list.length,gols:list.reduce((s,p)=>s+p.g1+p.g2,0),empates:list.filter(isDrawMatch).length,ultimo:Math.max(...list.map(p=>+p.ano))})).sort((a,b)=>b.j-a.j || b.gols-a.gols || a.jogo.localeCompare(b.jogo,"pt-BR"));
  table("tbl-games-head-to-head",["Confronto","Jogos","Gols","Empates","Última Copa"],h2h.slice(0,80).map(r=>[r.jogo,r.j,r.gols,r.empates,r.ultimo]),[1,2,3,4]);
  const biggest=rows.filter(p=>Math.abs(p.g1-p.g2)>0).map(p=>({p,margem:Math.abs(p.g1-p.g2),gols:p.g1+p.g2,winner:p.g1>p.g2?p.time1:p.time2,loser:p.g1>p.g2?p.time2:p.time1})).sort((a,b)=>b.margem-a.margem || b.gols-a.gols || +a.p.ano-+b.p.ano);
  table("tbl-games-biggest-wins",["Ano","Fase","Vencedor","Placar","Derrotado","Margem"],biggest.slice(0,80).map(r=>[r.p.ano,r.p.fase_normalizada,r.winner,r.p.placar,r.loser,r.margem]),[0,5]);
  const oppWins={};
  rows.forEach(p=>{
    if(isDrawMatch(p)) return;
    const winner=p.g1>p.g2?p.time1:p.time2, loser=p.g1>p.g2?p.time2:p.time1;
    const k=`${winner} sobre ${loser}`;
    const r=oppWins[k] ||= {winner,loser,v:0};
    r.v+=1;
  });
  table("tbl-games-opponent-wins",["Seleção","Adversário","Vitórias"],Object.values(oppWins).sort((a,b)=>b.v-a.v || a.winner.localeCompare(b.winner,"pt-BR")).slice(0,80).map(r=>[r.winner,r.loser,r.v]),[2]);
  const teamOpp={};
  rows.forEach(p=>[[p.time1,p.time2],[p.time2,p.time1]].forEach(([team,opp])=>{
    if(state.gameTeam && team!==state.gameTeam) return;
    const k=`${team}|${opp}`;
    const r=teamOpp[k] ||= {team,opp,j:0,v:0,e:0,d:0};
    r.j+=1;
    const res=gameResultForTeam(p,team);
    if(res==="Vitória") r.v+=1; else if(res==="Derrota") r.d+=1; else r.e+=1;
  }));
  table("tbl-games-team-opponents",["Seleção","Adversário","J","V","E","D"],Object.values(teamOpp).sort((a,b)=>b.j-a.j || b.v-a.v || a.team.localeCompare(b.team,"pt-BR")).slice(0,100).map(r=>[r.team,r.opp,r.j,r.v,r.e,r.d]),[2,3,4,5]);
  const streaks=gameStreakRows();
  table("tbl-games-streaks",["Seleção","Maior seq. vitórias","Maior seq. invicta","Maior seq. marcando","Maior seq. sem sofrer"],streaks.sort((a,b)=>b.invicto-a.invicto || b.vitorias-a.vitorias || a.team.localeCompare(b.team,"pt-BR")).map(r=>[r.team,r.vitorias,r.invicto,r.marcando,r.semSofrer]),[1,2,3,4]);
  table("tbl-games-negative-streaks",["Seleção","Maior seq. sem vencer","Maior seq. derrotas","Maior seq. sem marcar","Maior seq. sofrendo gol"],streaks.sort((a,b)=>b.semVencer-a.semVencer || b.derrotas-a.derrotas || a.team.localeCompare(b.team,"pt-BR")).map(r=>[r.team,r.semVencer,r.derrotas,r.semMarcar,r.sofrendo]),[1,2,3,4]);
  table("tbl-games",["Ano","Fase","Time 1","Placar","Time 2","Gols","Resultado","Estádio"],rows.slice().sort((a,b)=>+b.ano-+a.ano || b.idx-a.idx).map(p=>[p.ano,p.fase_normalizada,p.time1,p.placar,p.time2,p.g1+p.g2,isDrawMatch(p)?"Empate":"Com vencedor",p.estadio||"-"]),[0,5]);
}
function gameStreakRows(){
  const byTeam={};
  const games=filteredGames().slice().sort((a,b)=>a.ano-b.ano || a.idx-b.idx);
  games.forEach(p=>[p.time1,p.time2].forEach(t=>{
    if(!t || (state.gameTeam && t!==state.gameTeam)) return;
    const r=byTeam[t] ||= {team:t,vitorias:0,invicto:0,derrotas:0,semVencer:0,marcando:0,semMarcar:0,semSofrer:0,sofrendo:0,_v:0,_i:0,_d:0,_sv:0,_m:0,_sm:0,_ss:0,_sof:0};
    const res=gameResultForTeam(p,t);
    const gf=p.time1===t?p.g1:p.g2, ga=p.time1===t?p.g2:p.g1;
    r._v=res==="Vitória"?r._v+1:0; r.vitorias=Math.max(r.vitorias,r._v);
    r._i=res!=="Derrota"?r._i+1:0; r.invicto=Math.max(r.invicto,r._i);
    r._d=res==="Derrota"?r._d+1:0; r.derrotas=Math.max(r.derrotas,r._d);
    r._sv=res!=="Vitória"?r._sv+1:0; r.semVencer=Math.max(r.semVencer,r._sv);
    r._m=gf>0?r._m+1:0; r.marcando=Math.max(r.marcando,r._m);
    r._sm=gf===0?r._sm+1:0; r.semMarcar=Math.max(r.semMarcar,r._sm);
    r._ss=ga===0?r._ss+1:0; r.semSofrer=Math.max(r.semSofrer,r._ss);
    r._sof=ga>0?r._sof+1:0; r.sofrendo=Math.max(r.sofrendo,r._sof);
  }));
  return Object.values(byTeam).map(({_v,_i,_d,_sv,_m,_sm,_ss,_sof,...r})=>r);
}
...
function goalTeamEfficiencyStats(){
  const stats={};
  filteredGoalMatches().forEach(p=>{
    const s=scoreParts(p.placar);
    if(s[0]==="" || s[1]==="") return;
    ...
    [[p.time1,g1,g2],[p.time2,g2,g1]].forEach(([team,gp,gc])=>{
      if(!team || (state.goalTeam && team!==state.goalTeam)) return;
      const r=stats[team] ||= {team,jogos:0,gp:0,gc:0,marcou:0,semMarcar:0,clean:0,sofreu:0,seqMarcou:0,seqClean:0,maxSeqMarcou:0,maxSeqClean:0};
      r.jogos+=1; r.gp+=gp; r.gc+=gc;
      if(gp>0){r.marcou+=1; r.seqMarcou+=1;}else{r.semMarcar+=1; r.seqMarcou=0;}
      if(gc===0){r.clean+=1; r.seqClean+=1;}else{r.sofreu+=1; r.seqClean=0;}
      r.maxSeqMarcou=Math.max(r.maxSeqMarcou,r.seqMarcou);
      r.maxSeqClean=Math.max(r.maxSeqClean,r.seqClean);
    });
  });
  return Object.values(stats).map(r=>({
    ...r,
    sg:r.gp-r.gc,
    gpj:r.jogos?+(r.gp/r.jogos).toFixed(2):0,
    gcj:r.jogos?+(r.gc/r.jogos).toFixed(2):0,
    pctMarcou:r.jogos?+(r.marcou/r.jogos*100).toFixed(1):0,
    pctSemMarcar:r.jogos?+(r.semMarcar/r.jogos*100).toFixed(1):0,
    pctClean:r.jogos?+(r.clean/r.jogos*100).toFixed(1):0,
    pctSofreu:r.jogos?+(r.sofreu/r.jogos*100).toFixed(1):0,
  }));
}
function renderGoalTeamEfficiencyCharts(){
  const rows=goalTeamEfficiencyStats();
  renderMetricRankBars("ch-goals-off-eff", rows, "gpj", "Gols/jogo", COLORS[0], {decimals:2});
  renderTeamMetricGrouped("ch-goals-scoring-frequency", rows.sort((a,b)=>b.pctMarcou-a.pctMarcou || b.gpj-a.gpj), [
    {key:"pctMarcou",name:"Jogos marcando (%)",color:COLORS[0]},
    {key:"pctSemMarcar",name:"Jogos sem marcar (%)",color:COLORS[3]},
  ], "% dos jogos");
  renderTeamMetricGrouped("ch-goals-def-eff", rows.sort((a,b)=>b.pctClean-a.pctClean || a.gcj-b.gcj), [
    {key:"pctClean",name:"Clean sheets (%)",color:COLORS[2]},
    {key:"pctSofreu",name:"Jogos sofrendo gol (%)",color:COLORS[3]},
  ], "% dos jogos");
  table("tbl-goals-streaks",["Seleção","J","Gols/jogo","% marcou","Maior seq. marcando","% clean sheet","Maior seq. sem sofrer"],rows.sort((a,b)=>b.maxSeqMarcou-a.maxSeqMarcou || b.pctClean-a.pctClean).map(r=>[r.team,r.jogos,r.gpj.toFixed(2).replace(".",","),r.pctMarcou.toFixed(1).replace(".",",")+"%",r.maxSeqMarcou,r.pctClean.toFixed(1).replace(".",",")+"%",r.maxSeqClean]),[1,4,6]);
}
function matchKey(p){const s=scoreParts(p.placar); return `${p.ano}|${p.data||""}|${p.time1}|${p.time2}|${s[0]} - ${s[1]}`;}
function goalMatchKey(g){return `${g.ano}|${g.data||""}|${g.mandante}|${g.visitante}|${g.placar_mandante} - ${g.placar_visitante}`;}
function goalsForFilteredMatches(){
  const keys=new Set(filteredGoalMatches().map(matchKey));
  return ALL_GOALS.filter(g=>keys.has(goalMatchKey(g)));
}
function renderGoalContextCharts(rows){
  const matchGoals=groupBy(goalsForFilteredMatches(),goalMatchKey);
  const first=[];
  Object.values(matchGoals).forEach(list=>{
    const ordered=list.slice().sort((a,b)=>goalMinute(a.texto_original||goalMinuteText(a))-goalMinute(b.texto_original||goalMinuteText(b)));
    if(ordered[0] && (!state.goalTeam || ordered[0].selecao===state.goalTeam)) first.push(ordered[0]);
  });
  renderGoalRankList("ch-goals-first-teams", countRows(first,g=>g.selecao), COLORS[1]);
  const teamRows=Object.entries(groupBy(rows,g=>g.selecao)).map(([team,list])=>({
    team,
    apos75:list.filter(g=>num(g.minuto)>=76).length,
    acrescimos:list.filter(g=>num(g.minuto)>90 || String(g.minuto_label||"").includes("+")).length,
  })).sort((a,b)=>b.apos75-a.apos75 || a.team.localeCompare(b.team,"pt-BR"));
  renderTeamMetricGrouped("ch-goals-late-teams", teamRows, [
    {key:"apos75",name:"76 minutos em diante",color:COLORS[1]},
    {key:"acrescimos",name:"Acréscimos",color:COLORS[3]},
  ], "Gols");
  renderGoalImpactCharts(classifiedGoalImpacts());
}
function classifiedGoalImpacts(){
  const out=[];
  Object.values(groupBy(goalsForFilteredMatches(),goalMatchKey)).forEach(list=>{
    const ordered=list.slice().sort((a,b)=>goalMinute(a.texto_original||goalMinuteText(a))-goalMinute(b.texto_original||goalMinuteText(b)));
    if(!ordered.length) return;
    const first=ordered[0], home=first.mandante, away=first.visitante;
    const finalHome=num(first.placar_mandante), finalAway=num(first.placar_visitante);
    const winner=finalHome>finalAway?home:(finalAway>finalHome?away:"");
    const loserGoals=winner===home?finalAway:(winner===away?finalHome:0);
    let h=0, a=0, homeGoals=0, awayGoals=0, concededFirstBy="";
    ordered.forEach((g,idx)=>{
      const team=g.selecao;
      if(team!==home && team!==away) return;
      const beforeH=h, beforeA=a;
      if(team===home){h+=1; homeGoals+=1;}else{a+=1; awayGoals+=1;}
      if(idx===0) concededFirstBy=team===home?away:home;
      const beforeDiff=team===home ? beforeH-beforeA : beforeA-beforeH;
      const afterDiff=team===home ? h-a : a-h;
      const ownGoal=String(g.situacao||"").toLowerCase().includes("contra");
      out.push({
        ...g,
        abriuPlacar:idx===0,
        empate:afterDiff===0 && beforeDiff<0,
        lideranca:afterDiff>0 && beforeDiff<=0,
        virada:afterDiff>0 && concededFirstBy===team && idx>0,
        vitoria:winner===team && (team===home?homeGoals:awayGoals)===loserGoals+1,
        alterouResultado:Math.sign(beforeH-beforeA)!==Math.sign(h-a),
        golContra:ownGoal,
      });
    });
  });
  return out.filter(g=>!state.goalYear || String(g.ano)===String(state.goalYear)).filter(g=>!state.goalTeam || g.selecao===state.goalTeam);
}
function renderGoalImpactCharts(rows){
  const byTeam=Object.entries(groupBy(rows,g=>g.selecao)).map(([team,list])=>({
    team,
    abriu:list.filter(g=>g.abriuPlacar).length,
    empate:list.filter(g=>g.empate).length,
    virada:list.filter(g=>g.virada).length,
    vitoria:list.filter(g=>g.vitoria).length,
    mudou:list.filter(g=>g.alterouResultado).length,
  })).sort((a,b)=>b.vitoria-a.vitoria || b.abriu-a.abriu || a.team.localeCompare(b.team,"pt-BR"));
  renderTeamMetricGrouped("ch-goals-impact-teams", byTeam, [
    {key:"abriu",name:"Abriu placar",color:COLORS[1]},
    {key:"empate",name:"Empate",color:COLORS[2]},
    {key:"virada",name:"Virada",color:COLORS[5]},
    {key:"vitoria",name:"Vitória",color:COLORS[0]},
  ], "Gols");
  renderGoalRankList("ch-goals-decisive-players", countRows(rows.filter(g=>g.vitoria && g.jogador && g.jogador!=="Não informado" && g.jogador!=="Não identificado na fonte"),g=>g.jogador), COLORS[0]);
  const types=[
    ["Abriu placar", rows.filter(g=>g.abriuPlacar).length],
    ["Empate", rows.filter(g=>g.empate).length],
    ["Virada", rows.filter(g=>g.virada).length],
    ["Gol da vitória", rows.filter(g=>g.vitoria).length],
    ["Mudou resultado", rows.filter(g=>g.alterouResultado).length],
  ];
  renderGoalBar("ch-goals-impact-types", types, "Gols", COLORS[1]);
  const decisive=rows.filter(g=>g.vitoria);
  renderGoalBar("ch-goals-decisive-phase", Object.entries(countBy(decisive,g=>phaseLabel(g.fase))).sort((a,b)=>phaseSort(a[0],b[0])), "Gols da vitória", COLORS[0]);
}
function renderGoalCuriosities(rows){
  const matches=filteredGoalMatches();
  const byEdition=Object.entries(groupBy(matches,p=>p.ano)).sort((a,b)=>+a[0]-+b[0]);
  const koByEdition=byEdition.map(([ano,list])=>[ano, list.filter(p=>isKnockoutPhase(p.fase)).reduce((s,p)=>s+matchGoalsTotal(p),0)]).filter(r=>r[1]>0);
  renderGoalBar("ch-goals-ko-editions", koByEdition, "Gols no mata-mata", COLORS[0]);
  const lateByEdition=Object.entries(groupBy(rows,g=>g.ano)).map(([ano,list])=>({
    ano,
    apos75:list.filter(g=>num(g.minuto)>=76).length,
    acrescimos:list.filter(g=>num(g.minuto)>90 || String(g.minuto_label||"").includes("+")).length,
  })).sort((a,b)=>+a.ano-+b.ano);
  Plotly.react("ch-goals-late-editions",[
    {type:"bar",x:lateByEdition.map(r=>String(r.ano)),y:lateByEdition.map(r=>r.apos75),name:"76+ minutos",marker:{color:COLORS[1],cornerradius:4},hovertemplate:"<b>%{x}</b><br>76+ minutos: %{y}<extra></extra>"},
    {type:"bar",x:lateByEdition.map(r=>String(r.ano)),y:lateByEdition.map(r=>r.acrescimos),name:"Acréscimos",marker:{color:COLORS[3],cornerradius:4},hovertemplate:"<b>%{x}</b><br>Acréscimos: %{y}<extra></extra>"},
  ],layout({barmode:"group",hovermode:"closest",margin:{l:44,r:18,t:10,b:70},xaxis:{type:"category",tickangle:-25,automargin:true},yaxis:{title:"Gols"},legend:{orientation:"h",y:-.2}}),PCFG);
  table("tbl-goals-record-matches",["Ano","Fase","Jogo","Placar","Gols"],byEdition.map(([ano,list])=>{
    const p=list.slice().sort((a,b)=>matchGoalsTotal(b)-matchGoalsTotal(a))[0];
    return [ano,phaseLabel(p.fase),`${p.time1} x ${p.time2}`,p.placar,matchGoalsTotal(p)];
  }),[0,4]);
  const finals=DATA.copas.filter(c=>!state.goalYear || String(c.ano)===String(state.goalYear)).map(c=>{
    const p=c.partidas.find(x=>x.fase==="Final") || {};
    return {ano:c.ano,jogo:p.time1&&p.time2?`${p.time1} x ${p.time2}`:"-",placar:p.placar||editionRow(c.ano).placar_final||"-",gols:matchGoalsTotal(p)};
  }).filter(r=>!state.goalTeam || r.jogo.includes(state.goalTeam)).sort((a,b)=>b.gols-a.gols || +a.ano-+b.ano);
  table("tbl-goals-finals",["Ano","Final","Placar","Gols"],finals.map(r=>[r.ano,r.jogo,r.placar,r.gols]),[0,3]);
  const champions=DATA.copas.filter(c=>!state.goalYear || String(c.ano)===String(state.goalYear)).map(c=>{
    const e=editionRow(c.ano), r=c.classificacao_final.find(x=>x.selecao===e.campeao) || {};
    return {ano:c.ano,campeao:e.campeao,gp:num(r.gp),j:num(r.j),gpj:num(r.j)?+(num(r.gp)/num(r.j)).toFixed(2):0};
  }).filter(r=>r.campeao && (!state.goalTeam || r.campeao===state.goalTeam)).sort((a,b)=>a.gp-b.gp || a.gpj-b.gpj);
  table("tbl-goals-champions",["Ano","Campeão","Gols","Jogos","Gols/jogo"],champions.map(r=>[r.ano,r.campeao,r.gp,r.j,r.gpj.toFixed(2).replace(".",",")]),[0,2,3]);
  const exits=ARTILHEIROS_EDICAO.filter(r=>!state.goalYear || String(r[0])===String(state.goalYear)).flatMap(r=>{
    const c=DATA.copas.find(x=>+x.ano===+r[0]);
    return String(r[3]||"").split(",").map(t=>t.trim()).filter(Boolean).map(team=>{
      const pos=num((c?.classificacao_final||[]).find(x=>x.selecao===team)?.pos);
      return {ano:+r[0],artilheiro:r[1],selecao:team,gols:+r[2],pos};
    });
  }).filter(r=>r.pos && (!state.goalTeam || r.selecao===state.goalTeam)).sort((a,b)=>b.pos-a.pos || b.gols-a.gols || +a.ano-+b.ano);
  table("tbl-goals-topscorer-exits",["Ano","Artilheiro","Seleção","Gols","Posição final"],exits.map(r=>[r.ano,r.artilheiro,r.selecao,r.gols,`${r.pos}º`]),[0,3]);
}
function matchGoalsTotal(p){const s=scoreParts((p||{}).placar); return num(s[0])+num(s[1]);}
function isUnknownPlayer(g){return !g.jogador || g.jogador==="Não informado" || g.jogador==="Não identificado na fonte";}
function isMissingMinute(g){return !g.minuto || goalMinute(g.texto_original||goalMinuteText(g))>=999;}
function isMissingTech(g){
  const v=String(g.tipo_chute_pt||g.situacao||"").toLowerCase();
  return !v || v.includes("não detalhado") || v.includes("não identificado") || v.includes("não informado");
}
function goalQualityRows(rows){
  const matches=filteredGoalMatches();
  const expected=Object.entries(groupBy(matches,p=>p.ano)).map(([ano,list])=>[ano,list.reduce((s,p)=>s+expectedGoalsForQuality(p),0)]);
  const goalsByYear=groupBy(rows,g=>g.ano);
  return expected.map(([ano,total])=>{
    const list=goalsByYear[ano]||[];
    const withPlayer=list.filter(g=>!isUnknownPlayer(g)).length;
    const withMinute=list.filter(g=>!isMissingMinute(g)).length;
    const withTech=list.filter(g=>!isMissingTech(g)).length;
    return {
      ano,
      esperado:total,
      base:list.length,
      faltamGols:Math.max(total-list.length,0),
      semJogador:list.filter(isUnknownPlayer).length,
      semMinuto:list.filter(isMissingMinute).length,
      semTecnico:list.filter(isMissingTech).length,
      pctBase:total?+(list.length/total*100).toFixed(1):0,
      pctJogador:list.length?+(withPlayer/list.length*100).toFixed(1):0,
      pctMinuto:list.length?+(withMinute/list.length*100).toFixed(1):0,
      pctTecnico:list.length?+(withTech/list.length*100).toFixed(1):0,
    };
  }).sort((a,b)=>+a.ano-+b.ano);
}
function expectedGoalsForQuality(p){
  if(!state.goalTeam) return matchGoalsTotal(p);
  const s=scoreParts(p.placar);
  if(p.time1===state.goalTeam) return num(s[0]);
  if(p.time2===state.goalTeam) return num(s[1]);
  return 0;
}
function scorerDivergences(){
  return filteredGoalMatches().map(p=>{
    const c=DATA.copas.find(x=>+x.ano===+p.ano) || {ano:p.ano};
    const expected=matchGoalsTotal(p);
    const parsed=goalEvents(rawScorersForMatch(c,p),p).length;
    return {ano:p.ano,fase:phaseLabel(p.fase),jogo:`${p.time1} x ${p.time2}`,placar:p.placar,esperado:expected,marcadores:parsed,dif:parsed-expected};
  }).filter(r=>r.esperado!==r.marcadores).sort((a,b)=>+a.ano-+b.ano || Math.abs(b.dif)-Math.abs(a.dif));
}
function renderGoalQuality(rows){
  const q=goalQualityRows(rows);
  Plotly.react("ch-goals-coverage-editions",[
    {type:"bar",x:q.map(r=>String(r.ano)),y:q.map(r=>r.pctBase),name:"Gols na base",marker:{color:COLORS[0],cornerradius:4},hovertemplate:"<b>%{x}</b><br>Gols na base: %{y:.1f}%<extra></extra>"},
    {type:"bar",x:q.map(r=>String(r.ano)),y:q.map(r=>r.pctTecnico),name:"Tipo técnico",marker:{color:COLORS[2],cornerradius:4},hovertemplate:"<b>%{x}</b><br>Tipo técnico: %{y:.1f}%<extra></extra>"},
  ],layout({barmode:"group",hovermode:"closest",margin:{l:44,r:18,t:10,b:70},xaxis:{type:"category",tickangle:-25,automargin:true},yaxis:{title:"Cobertura (%)",range:[0,105]},legend:{orientation:"h",y:-.2}}),PCFG);
  Plotly.react("ch-goals-missing-editions",[
    {type:"bar",x:q.map(r=>String(r.ano)),y:q.map(r=>r.semJogador),name:"Sem jogador",marker:{color:COLORS[3],cornerradius:4},hovertemplate:"<b>%{x}</b><br>Sem jogador: %{y}<extra></extra>"},
    {type:"bar",x:q.map(r=>String(r.ano)),y:q.map(r=>r.semMinuto),name:"Sem minuto",marker:{color:COLORS[5],cornerradius:4},hovertemplate:"<b>%{x}</b><br>Sem minuto: %{y}<extra></extra>"},
    {type:"bar",x:q.map(r=>String(r.ano)),y:q.map(r=>r.semTecnico),name:"Sem tipo técnico",marker:{color:COLORS[1],cornerradius:4},hovertemplate:"<b>%{x}</b><br>Sem tipo técnico: %{y}<extra></extra>"},
  ],layout({barmode:"stack",hovermode:"closest",margin:{l:44,r:18,t:10,b:70},xaxis:{type:"category",tickangle:-25,automargin:true},yaxis:{title:"Gols"},legend:{orientation:"h",y:-.2}}),PCFG);
  table("tbl-goals-quality-editions",["Ano","Gols placar","Gols base","Cobertura","Sem jogador","Sem minuto","Sem tipo técnico"],q.map(r=>[r.ano,r.esperado,r.base,r.pctBase.toFixed(1).replace(".",",")+"%",r.semJogador,r.semMinuto,r.semTecnico]),[0,1,2,4,5,6]);
  const divs=scorerDivergences();
  table("tbl-goals-score-divergences",["Ano","Fase","Jogo","Placar","Gols placar","Marcadores","Dif."],divs.map(r=>[r.ano,r.fase,r.jogo,r.placar,r.esperado,r.marcadores,r.dif]),[0,4,5,6]);
  table("tbl-goals-unknown-players",["Ano","Fase","Jogo","Min","Seleção","Texto original"],rows.filter(isUnknownPlayer).slice().reverse().map(g=>[g.ano,phaseLabel(g.fase),`${g.mandante} x ${g.visitante}`,goalMinuteText(g),g.selecao,g.texto_original||"-"]),[0]);
  const total=rows.length || 1;
  const summary=[
    ["Gols auditados", rows.length],
    ["Sem jogador", rows.filter(isUnknownPlayer).length],
    ["Sem minuto", rows.filter(isMissingMinute).length],
    ["Sem tipo técnico", rows.filter(isMissingTech).length],
    ["Cobertura média", (q.reduce((s,r)=>s+r.pctBase,0)/(q.length||1)).toFixed(1).replace(".",",")+"%"],
    ["Divergências", divs.length],
  ];
  document.getElementById("goals-quality-summary").innerHTML=summary.map(([label,value])=>`<div class="quality-item"><b>${esc(value)}</b><span>${esc(label)}</span></div>`).join("");
}
function phaseLabel(phase){
  const p=String(phase||"").trim();
  if(!p) return "Não informado";
  if(p.toLowerCase()==="fase final") return "Fase de grupos";
...
function renderMetricRankBars(id, rows, key, title, color, opts={}){
  const host=document.getElementById(id);
  const chartRows=rows.slice().sort((a,b)=>opts.ascending ? a[key]-b[key] || a.team.localeCompare(b.team,"pt-BR") : b[key]-a[key] || a.team.localeCompare(b.team,"pt-BR"));
  host.style.height=`${Math.max(260, chartRows.length*24+80)}px`;
  const rev=chartRows.slice().reverse();
  const decimals=opts.decimals ?? 0;
  const suffix=opts.suffix || "";
  Plotly.react(id,[{type:"bar",orientation:"h",y:rev.map(r=>r.team),x:rev.map(r=>r[key]),marker:{color,cornerradius:3},text:rev.map(r=>`${Number(r[key]).toFixed(decimals).replace(".",",")}${suffix}`),textposition:"outside",cliponaxis:false,hovertemplate:`<b>%{y}</b><br>${title}: %{x:.${decimals}f}${suffix}<extra></extra>`}],layout({hovermode:"closest",margin:{l:118,r:42,t:10,b:45},xaxis:{title,automargin:true},yaxis:{automargin:true,tickfont:{size:11}}}),PCFG);
}
