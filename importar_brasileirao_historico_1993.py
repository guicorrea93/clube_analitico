"""Importa fases, participantes e jogos do Brasileiro 1993."""
...
AUDIT_CSV = ROOT / "data" / "brasileirao_historico_1993_partidas.csv"
SOURCE = "https://www.rsssf.org/tablesb/braz93.html"
...
CANONICAL.update({
    ...
    "ceara": "Ceara",
    "coritnhians": "Corinthians",
    "criciuma": "Criciuma",
    "deportiva": "Desportiva",
    "desportiva es": "Desportiva",
    "fortaleza": "Fortaleza",
    "gremio": "Gremio",
    "nautico": "Nautico",
    "parana": "Parana",
...
def team_name(value: str) -> str:
    clean = value.strip()
    clean = clean.replace("-BA", " BA").replace("-SP", "").replace("-ES", " ES")
    clean = re.sub(r"\s+", " ", clean)
    return canonical_name(clean)


def parse_games(block: str, fase: str, ordem: int, fase_tipo: str, expected: int, note: str = "") -> list[Game]:
    games: list[Game] = []
    current_round = None
    current_date = None
    game_no = 0
    for line in block.splitlines():
        ...
        if not m:
            continue
        game_no += 1
        games.append(Game(1993, fase, ordem, fase_tipo, current_round, game_no, current_date, team_name(m.group(1)), team_name(m.group(3)), int(m.group(2)), int(m.group(4)), SOURCE, note))
    if len(games) != expected:
        raise SystemExit(f"1993 {fase}: esperava {expected} jogos, encontrei {len(games)}")
    return games


def parse_1993() -> tuple[list[str], list[Game]]:
    text = read_pre(ROOT / "data" / "rsssf_braz93.html")
    first_start = text.find("FIRST STAGE")
    first_end = text.find("FINAL TABLE - FIRST PHASE")
    playoff_start = text.find("GROUPS C and D - PLAYOFF")
    second_start = text.find("SECOND PHASE", playoff_start)
    second_end = text.find("FINAL TABLE - SECOND PHASE")
    finals_start = text.find("FINALS", second_end)
    finals_end = text.find("SE Palmeiras", finals_start)

    games: list[Game] = []
    games += parse_games(text[first_start:first_end], "Primeira fase", 1, "grupo", 224)
    games += parse_games(text[playoff_start:second_start], "Playoff", 2, "mata_mata", 4, "Remo e Vitoria classificados por melhor campanha na primeira fase")
    games += parse_games(text[second_start:second_end], "Segunda fase", 3, "grupo", 24)
    games += parse_games(text[finals_start:finals_end], "Final", 4, "mata_mata", 2)

    participants = sorted({g.mandante for g in games[:224]} | {g.visitante for g in games[:224]})
    if len(participants) != 32:
        raise SystemExit(f"1993: esperava 32 participantes, encontrei {len(participants)}")
    if len(games) != 254:
        raise SystemExit(f"1993: esperava 254 jogos, encontrei {len(games)}")
...
def import_year(participants: list[str], games: list[Game]) -> None:
    ...
    try:
        create_tables(con)
        row = con.execute("SELECT edicao_nacional_id FROM dim_edicao_nacional WHERE temporada_id = 1993 AND competicao_nome = 'Campeonato Brasileiro'").fetchone()
        if not row:
            raise SystemExit("Edicao nacional 1993 nao encontrada.")
        edicao_id = row[0]
        ...
        for club in participants:
            con.execute("INSERT INTO fato_participante_nacional_historico (edicao_nacional_id, temporada_id, clube_id, fonte) VALUES (?, 1993, ?, ?)", (edicao_id, club_id(club), SOURCE))
        fase_ids = {}
        for g in games:
            if g.fase not in fase_ids:
                con.execute("INSERT INTO dim_fase_nacional_historica (edicao_nacional_id, temporada_id, fase_ordem, fase_nome, fase_tipo) VALUES (?, 1993, ?, ?, ?)", (edicao_id, g.fase_ordem, g.fase, g.fase_tipo))
                fase_ids[g.fase] = con.execute("SELECT last_insert_rowid()").fetchone()[0]
            con.execute(
                "INSERT INTO fato_partida_nacional_historica (edicao_nacional_id, temporada_id, fase_nacional_id, rodada, jogo, data, mandante_id, visitante_id, gols_mandante, gols_visitante, fonte, observacao) VALUES (?, 1993, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
def main() -> None:
    participants, games = parse_1993()
    import_year(participants, games)
    write_audit(games)
    print(f"1993: {len(participants)} participantes, {len({g.fase for g in games})} fases, {len(games)} jogos")
